...
class CreationTest : KoinTest {
    ...
    */
    ...
    private fun gitHubOAuthToken(): String {
        ...
        properties.load(
            javaClass.getResourceAsStream("/credentials.properties") ?: return System.getenv("GITHUB_OAUTH")
                ?: "broken-api-key"
        )
    ...
    fun `should generate accurate manifests`() {
         ...
         */
        fun assertManifestEquals(testType: String, manifestWithGenericIdentifier: ManifestWithGenericIdentifier) {
            assertEquals(
                json.decodeFromString(
                    readResource(
                        "/ExpectedManifest/$testType/${
                            manifestWithGenericIdentifier.genericIdentifier.substringBefore(
                                ":"
                            )
                        }/${manifestWithGenericIdentifier.genericIdentifier.substringAfter(":")}.json"
                    )
                ), manifestWithGenericIdentifier.manifestJson
            )
        }
        ...
        isTestingForGitHub = false.apply {
            isCurseForgeDistribution = true.apply {
                ...
                pureCurseForge.manifestsWithGenericIdentifiers.forEach { assertManifestEquals("pureCurseForge", it) }
            }
            ...
            pureModrinth.manifestsWithGenericIdentifiers.forEach { assertManifestEquals("pureModrinth", it) }
        }
